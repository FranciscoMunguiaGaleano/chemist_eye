...
CONFIG_FILE = '../config/cameratwo_tf_conf.yaml'
DETECTION_CONFIDENCE = 0.7

def call_chemisteyeone_speech_service():
    # Wait for the service to be available
    rospy.wait_for_service('/run_speech_service_two')  # Make sure the service is available

    try:
        # Create a service proxy to call the service
        run_bash_script = rospy.ServiceProxy('/run_speech_service_two', RunBashScript)
...
class YOLOv8Node:
    def __init__(self):
        ...
        rospy.init_node('yolov8_detection_node_two', anonymous=True)
        ...
        # Subscriptions
        self.image_sub = rospy.Subscriber('cameratwo/color/image_raw', Image, self.image_callback)
        self.array_sub = rospy.Subscriber('cameratwo/depth/array', Float32MultiArray, self.distance_callback)
        self.warning_colour_two_sub = rospy.Subscriber('cameratwo/warning_color_topic', String, self.colour_callback)

        # Publisher for annotated images
        self.annotated_image_pub = rospy.Publisher('cameratwo/annotated_image', Image, queue_size=10)

        # Publisher for TF transforms
        self.tf_broadcaster = tf.TransformBroadcaster()

        # Publisher for RViz markers
        self.marker_pub = rospy.Publisher('visualization_marker_two', Marker, queue_size=10)
    ...
    def delete_markers(self, human_id):
        ...
        delete_marker.header.frame_id = f"humantwo_{human_id}_frame"
    ...
    def broadcast_tf(self, x, y, z, human_id):
        ...
        tf_msg.child_frame_id = f"humantwo_{human_id}_frame"  # Unique frame for each person
    ...
    def update_markers(self, x, y, z, human_id):
        ...
        cylinder_marker.header.frame_id = f"humantwo_{human_id}_frame"
        ...
        sphere_marker.header.frame_id = f"humantwo_{human_id}_frame"
        ...
        shoulder_marker.header.frame_id = f"humantwo_{human_id}_frame"

...
LOCATIONS = [[-7.88, 0.48],[-1.43, 4.05],[-10.48, 4.05],[-10.48, 13.57],[-7.88, 13.57]]

class GUI():
    def __init__(self):
        self.mode = rospy.get_param('~rand_mode', False)
        self.root = tk.Tk()
        self.root.title('IR Camera Two tf calibrator')
        ...
        self.slider_x = ttk.Scale(self.root, from_=-20.0 * self.precision, to=20.0 * self.precision,
                                  length=self.slide_length, orient='horizontal', command=self.set_x,
                                  variable=self.current_x)
        self.slider_y = ttk.Scale(self.root, from_=-20.0 * self.precision, to=20.0 * self.precision,
        ...
        while not rospy.is_shutdown():
            ...
            try:
                self.br.sendTransform((x, y, z),
                                      ...
                                      rospy.Time.now(),
                                      "IR_cameratwo_link",  # Change to the appropriate frame name
                                      "world")
            except:
                pass
            self.rate.sleep()
    ...
    def load_config(self):
        config_path = os.path.join(os.path.dirname(__file__), '../config/ircameratwo_tf_conf.yaml')

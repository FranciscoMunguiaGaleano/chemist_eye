...
class GUI():
    def __init__(self):
        self.root = tk.Tk()
        self.root.title('Camera three tf calibrator')
        ...
        self.slider_x = ttk.Scale(self.root, from_=-10.0 * self.precision, to=10.0 * self.precision,
                                  length=self.slide_length, orient='horizontal', command=self.set_x,
                                  variable=self.current_x)
        self.slider_y = ttk.Scale(self.root, from_=-10.0 * self.precision, to=10.0 * self.precision,
        ...
        while not rospy.is_shutdown():
            ...
            try:
                self.br.sendTransform((x, y, z),
                                      ...
                                      rospy.Time.now(),
                                      "camerathree_link",  # Change to the appropriate frame name
                                      "world")
            except:
                pass
            self.rate.sleep()
    ...
    def load_config(self):
        config_path = os.path.join(os.path.dirname(__file__), '../config/camerathree_tf_conf.yaml')
